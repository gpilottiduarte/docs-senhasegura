...
| `identifier`  |  | String | Sim (para atualizar uma chave SSH). **Nota**: caso esse valor não seja fornecido durante a criação da chave SSH, nenhuma mudança via API poderá ser feita nos campos da chave. Neste caso, apenas a [inativação ou ativação da chave]({/* v */}3-32{/* docs */}{/* pt */}{/* a */}2a-pam-core-deactivate-or-activate-an-ssh-key) estará disponível através dos endpoints designados para isso. | Identificador da chave SSH.   | api-ssgr1144-caracteres-tags  | - |
